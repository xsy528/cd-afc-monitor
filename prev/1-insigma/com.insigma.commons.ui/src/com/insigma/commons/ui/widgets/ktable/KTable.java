...
import com.insigma.commons.ui.widgets.ktable.listener.KTablePopMenuListener;
...
public class KTable extends Canvas {
	...
	public KTable(Composite parent, int style) {
		...
		hideTxt = new Text(parent, SWT.NONE);
		// 不可见就无法设置焦点到此控件
		// hideTxt.setVisible(false);
			...
			private void repaintRelevantCells(GC gc, Point[] selections) {
				...
				for (int i = 0; i < selections.length; i++) {
					...
					for (int j = 0; j < getModel().getFixedHeaderColumnCount(); j++) {
						...
						Rectangle rowClip = new Rectangle(1, 1 + fixedHeight, fixedWidth,
					...
					for (int j = 0; j < getModel().getFixedHeaderRowCount(); j++) {
						...
						Rectangle rowClip = new Rectangle(1 + fixedWidth, 1, bounds.width - 1 - fixedWidth,
								fixedHeight);
	...
	protected void doCalculations() {
		...
		if (sb != null) {
			if (m_Model.getColumnCount() <= getFixedColumnCount() || allColumnsFullyVisible) {
				...
				sb.setMinimum(0);
				sb.setMaximum(
						m_Model.getColumnCount() - getFullyVisibleColCountAtEndOfTable() + 5 - getFixedColumnCount());
				sb.setIncrement(1);
				sb.setPageIncrement(1);
				sb.setThumb(5);
	...
	public void redraw(int firstCol, int firstRow, int numOfCols, int numOfRows) {
		...
		drawCells(gc, clipRect, firstCol, Math.min(firstCol + numOfCols, m_Model.getColumnCount()), firstRow,
	...
	public Point getValidCell(int colToCheck, int rowToCheck) {
		...
		if (colToCheck >= m_Model.getColumnCount() || rowToCheck >= m_Model.getRowCount() || colToCheck < 0
				|| rowToCheck < 0) {
			return found;
		}
	...
	protected void drawCell(GC gc, int col, int row, Rectangle rect) {
		...
		m_Model.getCellRenderer(col, row).drawCell(gc, rect, col, row, m_Model.getContentAt(col, row),
				...
				col == validClickedCell.x && row == validClickedCell.y, m_Model);
					...
					if (isHeaderCell(cell.x, cell.y)) {
						...
						if (m_FocusCol >= 0 && m_FocusRow >= 0) {
							...
							if (editor != null && rect.contains(click)
									...
									&& editor.isApplicable(KTableCellEditor.SINGLECLICK, this, m_FocusCol, m_FocusRow,
		...
		if (e.keyCode == SWT.HOME) {
			...
			if (!isRowSelectMode()) {
				if (newFocusCol > m_Model.getFixedHeaderColumnCount()) {
					...
					newFocusRow = newPt.y;
						...
						&& newFocusRow >= m_Model.getFixedHeaderRowCount() && e.keyCode == SWT.TAB) {
			...
			if (!isRowSelectMode()) {
				if (newFocusCol == -1) {
					...
					while (current.getParent() != null && !newFocusSet) {
						...
						for (int i = 0; i < parent.getTabList().length; i++)
							if (parent.getTabList()[i] == current && i < parent.getTabList().length - 1) {
								...
								while (newFocus instanceof Composite && ((Composite) newFocus).getTabList() != null
					...
					newFocusCol = next.x;
				} else if (m_FocusCol == m_Model.getColumnCount() - 1 && m_FocusRow != m_Model.getRowCount() - 1
						&& e.keyCode == SWT.TAB) {
					newFocusCol = m_Model.getFixedHeaderColumnCount();
					newFocusRow++;
				}
			...
			if (e.keyCode == SWT.DEL || e.keyCode == SWT.BS) {
				...
				return;
					...
					&& e.keyCode != SWT.CTRL && e.keyCode != SWT.ALT && (e.stateMask & SWT.CONTROL) == 0
				...
				if (isHeaderCell(cell.x, cell.y)) {
					...
					if (m_FocusCol >= 0 && m_FocusRow >= 0) {
						...
						if (editor != null && rect.contains(click)
								...
								&& editor.isApplicable(KTableCellEditor.DOUBLECLICK, this, m_FocusCol, m_FocusRow,
										click, null, e.stateMask))
							openEditorInFocus();
	...
	public int resizeColumnOptimal(int column) {
		...
		if (column >= 0 && column < m_Model.getColumnCount()) {
			...
			for (int i = Math.max(0, m_TopRow - INVISIBLE_ROWS_TO_REGARD_DURING_WIDTH_OPTIMIZATION); i < Math.min(
					m_TopRow + m_RowsVisible + INVISIBLE_ROWS_TO_REGARD_DURING_WIDTH_OPTIMIZATION,
	...
	protected void scrollToFocus() {
		if (m_FocusCol < 0 || m_FocusRow < 0 || m_FocusCol >= m_Model.getColumnCount()
	...
	protected void fireCellSelection(int col, int row, int statemask) {
		...
		for (int i = 0; i < cellSelectionListeners.size(); i++) {
			((KTableCellSelectionListener) cellSelectionListeners.get(i)).cellSelected(validCell.x, validCell.y,
					statemask);
		}
	}
	...
	protected void fireCellDoubleClicked(int col, int row, int statemask) {
		...
		for (int i = 0; i < cellDoubleClickListeners.size(); i++) {
			((KTableCellDoubleClickListener) cellDoubleClickListeners.get(i)).cellDoubleClicked(validCell.x,
					validCell.y, statemask);
		}
	}
	...
	protected void fireFixedCellDoubleClicked(int col, int row, int statemask) {
		Point validCell = getValidCell(col, row);
		for (int i = 0; i < cellDoubleClickListeners.size(); i++) {
			((KTableCellDoubleClickListener) cellDoubleClickListeners.get(i)).fixedCellDoubleClicked(validCell.x,
					validCell.y, statemask);
		}
	}
	...
	protected void fireFixedCellSelection(int col, int row, int statemask) {
		Point validCell = getValidCell(col, row);
		for (int i = 0; i < cellSelectionListeners.size(); i++) {
			((KTableCellSelectionListener) cellSelectionListeners.get(i)).fixedCellSelected(validCell.x, validCell.y,
					statemask);
		}
	}
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		if (m_Model != null) {
			...
			for (int i = m_Model.getFixedHeaderRowCount(); i < m_Model.getFixedHeaderRowCount()
					+ m_numRowsVisibleInPreferredSize && i < m_Model.getRowCount(); i++) {
				height += m_Model.getRowHeight(i);
				rowsVisible++;
			}
			...
			for (int i = m_Model.getFixedHeaderColumnCount(); i < m_Model.getFixedHeaderColumnCount()
					+ m_numColsVisibleInPreferredSize && i < m_Model.getColumnCount(); i++) {
	...
	public void resetPopMenu(List<KTablePopMenuListener> list) {
		if (null != this.getMenu() && !this.getMenu().isDisposed()) {
			this.getMenu().dispose();
		}
		this.setMenu(getPopMenu(list));
	}

	public Menu getPopMenu(List<KTablePopMenuListener> list) {
		Menu result = new Menu(this.getShell(), SWT.POP_UP);
		if (null != list) {
			for (final KTablePopMenuListener listener : list) {
				MenuItem cutItem = new MenuItem(result, SWT.PUSH);
				cutItem.setText(listener.getName());
				cutItem.addSelectionListener(new SelectionAdapter() {
					@Override
					public void widgetSelected(SelectionEvent arg0) {
						//						boolean updateFlag = listener.MenuClicked(m_FocusCol, m_FocusRow, m_Model.getDataListValue());
						//						if (updateFlag) {
						//							m_Model.updateTableValue();
						//							redraw();
						//						}
					}
				});
