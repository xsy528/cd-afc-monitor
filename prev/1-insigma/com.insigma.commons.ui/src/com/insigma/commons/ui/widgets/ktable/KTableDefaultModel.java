...
public abstract class KTableDefaultModel implements KTableModel {
	...
	public List<Object> getRowContents(int[] rows) {
		List<Object> result = new ArrayList<Object>();
		if (null != rows && rows.length > 0) {
			for (int row : rows) {
				Object obj = getRowContentAt(row);
				if (null != obj) {
					result.add(obj);
				}
			}
	...
	public Object getRowContentAt(int row) {
		...
		if (row > dataListValue.size() || row < 0) {
			throw new ApplicationException("无效行号。");
		}
		return dataListValue.get(row);
	}
