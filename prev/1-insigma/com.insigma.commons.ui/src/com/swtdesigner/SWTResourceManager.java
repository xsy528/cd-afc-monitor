...
public class SWTResourceManager {
	...
	protected static Image getImage(InputStream is, int w, int h) {
		Display display = Display.getCurrent();
		ImageData data = new ImageData(is);
		data = data.scaledTo(w < 1 ? data.width : w, h);
		if (data.transparentPixel > 0)
			return new Image(display, data, data.getTransparencyMask());
		return new Image(display, data);
	}
	...
	public static Image getImage25x25(Class<?> clazz, String path) {
		return getImage(clazz, path, 25, 25);
	}

	public static Image getImage16x16(Class<?> clazz, String path) {
		return getImage(clazz, path, 16, 16);
	}

	public static Image getImage(Class<?> clazz, String path, int w, int h) {
		String key = clazz.getName() + '|' + path + w * h;
		Image image = m_ClassImageMap.get(key);
		if (image == null) {
			try {
				if (path.length() > 0 && path.charAt(0) == '/') {
					String newPath = path.substring(1, path.length());
					image = getImage(new BufferedInputStream(clazz.getClassLoader().getResourceAsStream(newPath)), w,
							h);
				} else {
					image = getImage(clazz.getResourceAsStream(path), w, h);
				}
				m_ClassImageMap.put(key, image);
			} catch (Exception e) {
				image = getMissingImage();
	 ...
	 */
	public static void disposeGroupImages(String section) {
	...
	/**
	 * Dispose cached image with path
	 * @param section the section do dispose
	 */
	public static void disposeImage(String path) {
		String key = "default" + '|' + path;
		Image image = m_ClassImageMap.remove(key);
		//释放图片资源
		//		if (image != null) {
		//			image.dispose();
		//		}
	 ...
	 * @param section the section do dispose
	 */
	public static void disposeImages(Class<?> clazz, String path) {
		String key = clazz.getName() + '|' + path;
		Image image = m_ClassImageMap.remove(key);
		if (image != null) {
			image.dispose();
