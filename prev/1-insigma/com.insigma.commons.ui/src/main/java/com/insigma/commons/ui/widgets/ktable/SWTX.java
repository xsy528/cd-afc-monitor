...
public class SWTX {
	...
	private static HashMap<String, Color> m_ColorMap = new HashMap<String, Color>();

	static {
		Color color1 = SWTResourceManager.getColor(new RGB(223, 227, 237));//202, 209, 230));
		m_ColorMap.put(COLOR_BGFOCUS, color1);//202, 209, 230));
		Color color2 = SWTResourceManager.getColor(new RGB(182, 189, 210));//202, 209, 230));
		m_ColorMap.put(COLOR_FIXEDHIGHLIGHT, color2);
	}
	...
	public static Color getColor(String colorKey) {
		return m_ColorMap.get(colorKey);
	}
		...
		if (textSize.x > width) {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				while (breakOffset < lines[i].length()) {
					String lPart = lines[i].substring(breakOffset,
							Math.min(breakOffset + cutoffLength, lines[i].length()));
	...
	public static void drawButtonUp(GC gc, String text, int textAlign, Image image, int imageAlign, int x, int y, int w,
			int h, Color face, Color shadowHigh, Color shadowNormal, Color shadowDark, int leftMargin, int topMargin) {
		...
		try {
			...
			drawTextImage(gc, text, textAlign, image, imageAlign, x + 1 + leftMargin, y + 1 + topMargin,
					w - 3 - leftMargin, h - 3 - topMargin);
		} finally {
			gc.setForeground(prevForeground);
	...
	public static void drawButtonDown(GC gc, String text, int textAlign, Image image, int imageAlign, int x, int y,
		...
		try {
			...
			gc.setForeground(prevForeground);
			drawTextImage(gc, text, textAlign, image, imageAlign, x + 2 + leftMargin, y + 2 + topMargin,
					w - 3 - leftMargin, h - 3 - topMargin);
		} finally {
			gc.setForeground(prevForeground);
			gc.setBackground(prevBackground);
	...
	public static void drawButtonDown(GC gc, String text, int textAlign, Image image, int imageAlign, int x, int y,
		...
		Display display = Display.getCurrent();
		drawButtonDown(gc, text, textAlign, image, imageAlign, x, y, w, h, face,
				display.getSystemColor(SWT.COLOR_WIDGET_NORMAL_SHADOW), 2, 2);
	}
