...
public class WZACCMetroNodeStatusServiceImpl extends Service implements IMetroNodeStatusService {

	@Override
	public TmoItemStatus getTmoItemStatus(Boolean isDay, long nodeId) {

		StringBuffer sb = new StringBuffer();
		sb.append("from TmoItemStatus t where 1=1 and t.nodeId=? ");
					...
					for (MetroStation metroStation : subStationNodes) {
						...
						for (MetroDevice metroDevice : metroDeviceList) {
							...
							if (deviceMaps.containsKey(Long.valueOf(metroDevice.getId().getDeviceId()))) {
								...
								temp = getEquStatusViewItem(tmoItemStatus, filterForm, isOnLine);
	...
	private EquStatusViewItem getEquStatusViewItem(TmoItemStatus tmoItemStatus, EquStatusFilterForm filterForm,
												   boolean fatherStatus) {
		EquStatusViewItem temp = new EquStatusViewItem();
		List<Short> statusLevel = filterForm.getStatusLevelList();
		long normal = 0;
		long warning = 0;
	...
	@Override
	public List<StationStatustViewItem> getAllStationStatusView(List<MetroStation> metroStations) {
		///获取通信前置机状态
		Number onLine = (Number) Application.getData(ApplicationKey.STATUS_COMMUNICATION);
		...
		//查询车站状态
		List<TmoItemStatus> stationStatus = null;
		try {
			stationStatus = this.commonDAO.getEntityListHQL(
		...
		//查询设备状态
		List<TmoItemStatus> deviceStatus = null;
		try {
			deviceStatus = this.commonDAO.getEntityListHQL(
		...
		List<StationStatustViewItem > stationStatustViewItems = new ArrayList<>();
		for (MetroStation station:metroStations) {
			int normalEvent = 0;
			int warnEvent = 0;
			int alarmEvent = 0;
			Integer stationId = station.getId().getStationId();
			Short lineId = station.getId().getLineId();
			StationStatustViewItem viewItem = new StationStatustViewItem();
			viewItem.setLineId(lineId);
			viewItem.setStationId(stationId);
			viewItem.setNodeId(station.getId().getStationId());
			viewItem.setNodeType(station.getNodeType());
			if (stationMaps.containsKey(viewItem.getNodeId())) {
				TmoItemStatus tmoItemStatus = stationMaps.get(station.getId().getStationId().longValue());
			...
			stationStatustViewItems.add(viewItem);
		}
		return stationStatustViewItems;
	}
