...
%//	U8_t                                                            SAK;                            /*卡种*/    
%//	U8_t                                                            MainCardType;                            /*主卡类型 直接取卡上数据，M1叫卡类型，CPU卡叫卡主类型*/      
%//	U8_t                                                            SubCardType;                            /*子卡类型 直接取卡上数据，M1填0，CPU卡取卡子类型*/     
%//     U8_t                                                            CardVer;                            /*卡内版本号*/   
...
struct TicketComm_t{													
	...
	string						        TicketLogicID<20>;                  /*票卡逻辑号 公交CPU卡取应用序列号8个字节，M1卡取卡号(城市代码2个字节+行业代码2个字节+发行流水号4个字节)*/
...
%// ---- 银行卡支付结构
struct BankCardPayment_t{										
        string					BankCode<32>;                       /*银行代码*/
        string					BIZCode<32>;			    /*商户号*/
        string					PosNo<32>;                          /*终端编号*/
        string					BankCardCode<32>;                   /*银行卡号*/
        SN_t					BankTransSN;                        /*银行交易流水号*/
};
...
struct BankCardDeduction_t{											
	TestFlag_t						TestFlag;		            /*测试标记*/
	BankCardPayment_t					BankCardPayment;		    /*银行卡支付结构*/
        ValueCent_t						TransactionValue;                   /*扣值金额*/
...
%// ---- M1卡地铁应用启用
%// struct YKTMetroStart_TX{													
%//         TicketComm_t						TicketComm;						/*票卡公共结构*/
%//	   YKTMetroStart_t						YKTMetroStart;					/*票卡地铁应用启用结构*/
%// };
...
struct NoteBoxInfo_t{
	...
	ARCount_t					     NbofNote;                             /* 纸币数量*/
	ValueCent_t					     TotalValueofCash;	                   /* 纸币金额*/
};
...
struct ISMSnapshotInfo_t{														
	...
	ValueCent_t							TotalValueofBankNotebox;                /*纸币箱总金额*/
        NoteBoxInfo_t                                                   NoteBoxInfos<>;                         /*纸币信息列表*/
        ARCount_t							TotalNbofBNAInsert;                     /*接受纸币张数*/
	ValueCent_t							TotalValuebyInsertedBanknote;           /*纸币交易金额*/
	ARCount_t							TotalNbofValidACCCSCRead;               /*储值票有效读写次数*/
...
%// ---- 日终库存差异文件结构
struct TicketStockDifferenceFile_t{			
        LineID_t				        LineID;							/* 线路编号 */
	StationID_t					StationID;						/* 车站编号 */		
        Date2_t						BusinessDate;						/* 运营日 */										
	TicketDifferenceStockList_t		        TicketDifferenceStockList<>;				/* 车票库存差异列表结构 */
};
