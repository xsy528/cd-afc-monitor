...
%*   2018/08/14    *    华科峰           * BOMSnapshotInfo_t结构体添加TotalValueofDeposit字段
%*   2018/08/14    *    华科峰           * 修改TVMSnapshotInfo_t结构体调整TotalValueofCashOut字段位置
%*   2018/08/14    *    华科峰           * 修改TicketStockInOutDetailList_t结构体中单据编号为BillCode<20>
%*   2018/08/14    *    华科峰           * 修改TicketStockInOutDetailList_t结构体新增ValidDate字段
%*   2018/08/14    *    华科峰           * 修改TicketComm_t结构体中预留字段为SAMSN
%*   2018/08/14    *    华科峰           * 修改ARComm_t结构体添加QRUDSNStart、QRUDSNEnd、Reserved1、Reserved2、Reserved3、Reserved4字段
%*   2018/08/14    *    华科峰           * 修改AGMSnapshotInfo_t结构体替换原先字段TotalNbofTestEntry、TotalNbofTestExit、TotalValueTestCollectedbySJT、TotalValueTestCollectedbyACCCSC、TotalValueTestCollectedbyYKTCSC、TotalValueTestCollectedbyMobile为TotalNbofBankCardEntry、TotalNbofBankCardExit、TotalValuebyBankCard、TotalNbofQRCodeEntry、TotalNbofQRCodeExit、TotalValuebyQRCode
%*   2018/08/14    *    华科峰           * 修改TVMSnapshotInfo_t结构体替换原先字段Reserved1、Reserved2为TotalNbofIssuedSJTByBank、TotalValueofIssueSJTByBank
%**************************************************************************************************************/
...
struct NamedCPUSale_TX{												
	TicketComm_t						TicketComm;				/*票卡公共结构*/
	CPUInitComm_t						CPUInitComm;				/*储值票卡初始化结构*/
	CPUCardSale_t				CPUInitComm_t		CPUCardSale;			/*储值票发售结构*/
...
struct ARComm_t{												
	...
	SN_t								BankUDSNStart;                      /*上一个寄存器文件关联的最后一笔银联支付交易记录的UDSN+1*/
	SN_t								BankUDSNEnd;                        /*当前寄存器文件关闭时所关联的最后一笔银联支付交易记录的UDSN*/
	SN_t								QRUDSNStart;                      /*上一个寄存器文件关联的最后一笔二维码支付交易记录的UDSN+1*/
	SN_t								QRUDSNEnd;                        /*当前寄存器文件关闭时所关联的最后一笔二维码支付交易记录的UDSN*/
	SN_t								Reserved1;                          /*预留1*/
	SN_t								Reserved2;                           /*预留2*/
	SN_t								Reserved3;                           /*预留3*/
	SN_t								Reserved4;                           /*预留4*/
...
struct AGMSnapshotInfo_t{													
       ...
       ARCount_t							TotalNbofBankCardEntry;                 /*银行卡进站数*/
       ARCount_t							TotalNbofBankCardExit;                  /*银行卡出站数*/
       ValueCent_t							TotalValuebyBankCard;                   /*银行卡交易金额*/
       ARCount_t							TotalNbofQRCodeEntry;                   /*二维码进站数*/
       ARCount_t							TotalNbofQRCodeExit;                    /*二维码出站数）*/
       ValueCent_t							TotalValuebyQRCode;                     /*二维码交易金额*/
...
struct TVMSnapshotInfo_t{											
	...
	ARCount_t							TotalNbofValidSJTCSCRead;               /*单程票有效读写次数*/
        ARCount_t							TotalNbofValidACCCSCRead;               /*储值票有效读写次数*/
	ARCount_t							TotalNbofValidYKTCSCRead;               /*一卡通有效读写次数*/
 	ARCount_t							TotalNbofIssuedSJTByBank;               /*单程票发售数量（银联）*/
	ValueCent_t							TotalValueofIssueSJTByBank;             /*单程票发售金额（银联）*/
        ARCount_t							Reserved3;                              /* 预留3 */
