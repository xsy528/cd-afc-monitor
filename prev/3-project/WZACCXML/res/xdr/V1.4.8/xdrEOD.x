...
%*   2017/01/18    *    华科峰           *     修改JoinConcessionDefinition_t中JoinConcessionIndustryScope的中文说明 
%*   2017/01/18    *    华科峰           *     修改EOD_TVM_GUIParam_t，添加界面代码说明
%*   2017/08/20    *    陈锦鲂           *     TicketTypeMap_t结构体中YKTTicketType类型修改为U16_T
%*   2017/11/03    *    陈锦鲂           *     增加FunctionList_t结构体，RoleFunctionList_t结构体增加RoleDesc字段，
%*											   OperatorList_t结构体注释掉EQPFuncID_t，OperatorRight_t结构体增加FunctionList_t字段
%*                                             将操作员权限参数中的OperatorList_t结构移动至操作员基本参数中，注释OperatorList_t结构体中的OperatorID_t字段
%*   2017/11/23    *    陈锦鲂           *     TicketDefinition_t结构体中的RefundAuthorized字段类型调整为U8_t
%*   2017/03/27    *    华科峰           *     删除EODParameter_t结构体中关于设备软件及TP软件的定义
%*   2017/04/03    *    华科峰           *     修改OperInfo_t结构体，添加OperatorName字段
%*   2017/04/03    *    华科峰           *     合并OperatorList_t架构体到OperInfo_t
%*   2017/04/03    *    华科峰           *     修改EODParameter_t结构体中关于RG通行控制参数格式的定义
%*   2017/06/07    *    华科峰           *     修改EOD_ISMParam_t结构体添加LanguageDisplayWindow、CheckCardDisplayWindow、InfoQueryDisplayWindow字段
%*   2017/06/07    *    华科峰           *     修改TVMGUI_t结构体添加ReducePictureWidth，ReducePictureHeight字段
%*   2017/06/07    *    华科峰           *     修改NumbersBtn_t结构体TicketQuantity类型为U16_T
%*   2017/07/31    *    华科峰           *     修改TVMControlDefinition_t结构体MaxAmountOfBuyTicket类型为U16_T
%*   2017/07/31    *    华科峰           *     修改TVMGUI_t、EOD_ISMParam_t结构体ADPlayType类型为U8_T
%*   2017/07/31    *    华科峰           *     修改TicketCatalog_t结构体TicketUnit字段类型为U8_T
%**************************************************************************************************************/
...
struct TransferStationList_t {                                                                   
	StationID_t                              StationID;                              /* 换乘区域内车站 */
	Boolean_t                                EnableFlag;                            /* 启用标志位 */	
};
...
struct TransferStation_t {                                                                   
	...
	TransferStationList_t                    TransferStationLists<>;                /* 换乘区域内车站列表 */
};
...
struct EOD_CalendarParam_t { 
	...
	SecondsSinceMidNight_t                   RunEndTime;                            /* 运营结束时间 */
        SecondsSinceMidNight_t                   BalaStartTime;                         /* 系统结算起始时间 */
        SecondsSinceMidNight_t                   BalaEndTime;                           /* 系统结算结束时间 */
	SecondsSinceMidNight_t                   ClearEndTime;                          /* 系统清算截至时间 */
	SecondsSinceMidNight_t                   AccEndTime;                            /* 系统对账时间 */
	Seconds_t                                UrgrelDelayTime;                       /* 紧急放行模式后票卡延长使用期 */
	Seconds_t                                TrainfaultDelayTime;                   /* 列车故障模式后票卡延长使用期 */
	Seconds_t                                ModeSenTime;                           /* 模式后果敏感期 */
...
struct TicketDefinition_t{                                                          
	...
	U8_t                                	 RefundAuthorized;                      /* 退票方式 0：不允许退票，1：只允许售票站退票 2-允许退票 */
...
struct JoinConcessionDefinition_t{	                                           
        ConcessionID_t                           JoinConcessionID;                      /* 联程优惠代码 */
	IndustryScope_t                          JoinConcessionIndustryScope;           /* 联程优惠换乘范围 */	
...
struct TVMControlDefinition_t{	                                           
	RecordsInFile_t                          SingleFileMaxDeals;                    /* 单个文件最大交易数量 */
	U16_t     	                         MaxAmountOfBuyTicket;                  /* 单次购票最大数量 */
...
struct TVMGUI_t{                                                           
	...
	DisplayWindow_t                          LineZoneDisplayWindow;                 /* 路网图片显示坐标 */
        CoordinateUnit_t                         ReducePictureWidth;                   /* 线网图片缩小后宽度 */
        CoordinateUnit_t                         ReducePictureHeight;                  /* 线网图片缩小后高度 */
	...
	U8_t                                     ADPlayType;                            /* 广告播放方式 0从头开始，1继续播放（预留） */	
...
struct EOD_ISMParam_t{                                                        
	...
	Seconds_t                                MaxIdleToPlayAD;                       /* 进入广告模式的最大空闲时间（预留） */	
	TopBoard_t                               TopBoards<>;                           /* 顶板列表 */	
	DisplayWindow_t                          LanguageDisplayWindow;                 /* 中英文按钮显示信息 */
	DisplayWindow_t                          CheckCardDisplayWindow;		/* 验卡按钮显示信息 */
	DisplayWindow_t                          InfoQueryDisplayWindow;		/* 信息查询按钮显示信息 */
        DisplayWindow_t                          AddValueDisplayWindow;                 /* 充值按钮显示坐标 */
	AddValueBtn_t                            AddValueBtns<>;                        /* 充值选择按钮列表 */
};
...
%// ---- 设备软件参数文件
struct EOD_SLESoftParam_t{                                                        
	ResourceFile_t                           TVMSoft;                               /* TVM软件文件 */
	ResourceFile_t                           BOMSoft;                               /* BOM软件文件 */
	ResourceFile_t                           AGMSoft;                               /* AGM软件文件 */
	ResourceFile_t                           ISMSoft;                               /* ISM软件文件 */
	ResourceFile_t                           PCASoft;                               /* PCA软件文件 */
};

%// ---- TP软件参数文件
struct EOD_TPSoftParam_t{                                                        
	ResourceFile_t                           TPSoft;                               /* TP软件文件 */
};
...
union EODParameter_t switch (EODComponentType_t eodCOMType) {                          
    ...
    case EOD_SYSNetwork:                                                             /* EOD: 线网信息参数 */
...
struct OperInfo_t{                                                       
	OperatorID_t                             OperatorID;                            /* 用户编号 */
        UnicodeString_t                          OperatorName;                          /* 用户名称*/
...
struct FunctionList_t{                                                       
	EQPFuncID_t                           EQPFuncID;                           /* 功能编号 */
        UnicodeString_t                       EQPFuncDesc;                       /* 功能名称*/
};

%// ---- 操作员权限参数
struct OperatorRight_t{                                                       
    RoleFunctionList_t                       RoleFunctions<>;                       /* 角色功能列表*/
	FunctionList_t			     FunctionLists<>;						/* 功能列表*/
};
