...
%*   2017/09/21    *    陈锦鲂           *     增加BLKACCFullList_t、BLKACCIncList_t、BLKACCSectList_t、SJTRecycleCondList_t、
%*												WaiveDateList_t、FunctionList_t结构体，RUNParameter_t结构体内容调整，RoleFunctionList_t结构体增加RoleDesc字段，
...
union EODParameter_t switch (EODComponentType_t eodCOMType) {                          
    ...
    case EOD_SLESoft:                                                               /* EOD:设备软件参数 */
         EOD_SLESoftParam_t            EOD_SLESoftParam;

    case EOD_TPSoft:                                                                /* EOD: TP软件参数 */
         EOD_TPSoftParam_t              EOD_TPSoftParam;
...
%// ---- 全量明细黑名单文件
struct BLKACCFullList_t{                                                         
	BLKACCFull_t                    BLKACCFullList<>;                             /* 增量明细黑名单结构 */
};

%// ---- 增量明细黑名单文件
struct BLKACCIncList_t{                                                         
	BLKACCInc_t                     BLKACCIncList<>;                               /* 增量明细黑名单结构 */
};

%// ---- 号段黑名单文件
struct BLKACCSectList_t{                                                         
	BLKACCSect_t                    BLKACCSectList<>;                               /* 增量明细黑名单结构 */
};

%// ---- 员工卡全量明细黑名单文件
struct BLKStaffFullList_t{                                                         
	BLKStaffFull_t                 BLKStaffFullList<>;                               /* 增量明细黑名单结构 */
};

%// ---- 单程票回收条件文件
struct SJTRecycleCondList_t{                                                         
	SJTRecycleCond_t                 SJTRecycleCondList<>;                          /* 增量明细黑名单结构 */
};

%// ---- 模式履历文件
struct WaiveDateList_t{                                                         
	WaiveDate_t                 WaiveDateList<>;                              		/* 增量明细黑名单结构 */
};
...
%// ---- 操作员基本信息文件
struct OperInfoList_t{                                                       
	OperInfo_t						OperInfoList<>;									/* 操作员基本信息列表 */
};
