...
%*   2018/02/09    *    华科峰           *     删除EODComponentType_t中EOD_SLESoft，EOD_TPSoft的定义
%*                                             删除RUNComponentType_t中RUN_STATIONEQUNODE的定义
%*                                             删除FileHeaderTag_t中FILE_EOD_SLESoft，FILE_EOD_TPSoft定义，并添加FILE_TVM_Soft，FILE_BOM_Soft，FILE_PCA_Soft，FILE_AGM_Soft，FILE_TP_Soft定义
%*   2018/03/16    *    华科峰           *     修改enum DeviceType_t定义
%*   2018/04/12    *    华科峰           *     修改DateTypeID_t定义
%*   2018/04/13    *    华科峰           *     修改enum SAMType_t定义
%*   2018/04/13    *    华科峰           *     FileHeaderTag_t结构中添加PCA软件程序文件
%*   2018/05/07    *    华科峰           *     修改enum ComType_t定义
%*   2018/06/19    *    华科峰           *     添加ContainerOperatePos_t定义
%*   2018/06/22    *    华科峰           *     修改enum ChipType_t结构体增加CT_UNIONPAY、CT_QRCODE
%*   2018/06/22    *    华科峰           *     修改enum TicketFamilyType_t结构体增加TF_UNIONPAY、TF_QRCODE
%*   2018/06/22    *    华科峰           *     修改enum TransactionType_t，添加银联及二维码相关交易
%*   2018/07/04    *    华科峰           *     修改enum FileHeaderTag_t,添加二维码交易文件
%*   2018/07/14     *   华科峰           *     修改enum ComType_t定义，添加库存收益类及BOM类报文定义
%*   2017/07/31    *    华科峰           *     修改DeviceOwnerType_t定义类型为U16_t
%*   2018/08/01    *    华科峰           *     修改enum ComType_t定义，新增COM_TICKETBOX_QUERY、COM_TICKETBOX_UPLOAD定义
%*   2018/08/01    *    华科峰           *     添加TicketBoxType_t定义
%**************************************************************************************************************/
...
%// 银行卡部分
	TX_BankCardDeduction                    = 0x60, /* 银行卡扣款 */
        TX_BankCardSurcharge                    = 0x61, /* 银行卡更新 */
	TX_BankCardEntry                        = 0x62, /* 银行卡进站 */
	TX_BankCardExit                         = 0x63, /* 银行卡出站 */
%// ES部分
...
%// 二维码部分
        TX_QRCodeSurcharge                      = 0x90, /* 二维码更新 */
	TX_QRCodeEntry                          = 0x91, /* 二维码进站 */
	TX_QRCodeExit                           = 0x92  /* 二维码出站 */ 
};
...
enum FileHeaderTag_t {                          
	...
	FILE_QRCODE_TRANSACTION		       = 0x0C,  /* 二维码交易文件 */
	...
	FILE_EOD_PCA			       = 0x2D,  /* EOD: PCA参数文件 */
        FILE_TVM_Soft                          = 0x2E,  /* TVM软件程序文件 */
        FILE_BOM_Soft                          = 0x2F,  /* BOM软件程序文件 */
        FILE_AGM_Soft                          = 0x30,  /* AGM软件程序文件 */
        FILE_ISM_Soft                          = 0x31,  /* ISM软件程序文件 */
	FILE_TP_Soft                           = 0x32,  /* TP软件程序文件 */
	FILE_PCA_Soft                          = 0x33,  /* PCA软件程序文件 */
...
%// 日期类型
enum DateTypeID_t{                              
	DTI_WORKDAY                                = 0x00,  /* 正常工作日 */
	DTI_SATURDAY                               = 0x01,  /* 星期六 */
	DTI_SUNDAY                                 = 0x02,  /* 星期天 */
	DTI_SPECIALDAY                             = 0x03   /* 特殊日期 */
};
...
enum ComType_t{											
	...
	COM_TICKETBOX_QUERY					= 0x1310,   /* 查询设备票箱数据 */
	COM_TICKETBOX_UPLOAD					= 0x1311,   /* 设备票箱数据上传 */
	...
	COM_UPLOAD_TICKET_STOCK_NOTIFY			        = 0x1603,   /* 通知下级上传票卡库存信息 */
	COM_UPLOAD_TICKET_STOCK					= 0x1604,   /* 上传票卡库存信息 */
	COM_UPLOAD_TICKET_INCOME				= 0x1605,   /* 上传票款收益信息 */
	COM_UPLOAD_CHANGESHIFTS_RECORD				= 0x1606,   /* 客值交接班信息上报 */
        COM_TICKET_FILLINGMONEY_NOTICE				= 0x1607,   /* 短款补款信息通知 */
	COM_UPLOAD_TICKET_FILLINGMONEY				= 0x1608,   /* 上传短款补款信息 */
        COM_UPLOAD_CASH_FORM				        = 0x1609,   /* 设备现金登记日报上报 */
        COM_UPLOAD_SELLER_STATEMENTS				= 0x1610,   /* BOM售票员结算单上报 */
        COM_UPLOAD_STATION_REVENUEDAILY				= 0x1611,   /* 车站营收日报上报 */

        COM_UPLOAD_STATION_IMMEDIATE_REFUND			= 0x1612,   /* 特殊情况车票即时退款信息上报 */
        COM_UPLOAD_TICKET_INFO    				= 0x1613,   /* 车站票卡售存信息上报 */
	COM_UPLOAD_TICKET_INVENTORY				= 0x1614,   /* 车站票卡盘点信息上报 */
        COM_UPLOAD_RESERVEFUNDS_INFO				= 0x1615,   /* 车站备用金清点信息上报 */
        COM_UPLOAD_SPECFUNDS_INFO				= 0x1616,   /* 特殊情况票款信息上报 */
        COM_UPLOAD_PAPERTICKET_SALEINFO				= 0x1617,   /* 纸票发售信息上报 */
        COM_UPLOAD_SLECASH_OPERINFO				= 0x1618,   /* 设备补币、清点信息上报 */
        COM_UPLOAD_CUSTOMERSERVICE_STATEMENTS			= 0x1619,   /* 客服结算单信息上报 */
	...
	COM_UPLOAD_WEALTICKETINFO                               = 0x1711,   /* 福利票发售信息上传 */
        COM_UPLOAD_PASSENGERAFFAIRINFO                          = 0x1712,   /* 车站乘客事务处理信息上报 */
        COM_STATION_NONIMMEDIATE_REFUND_APPLY                   = 0x1713,   /* 车站非即时退款申请 */
...
typedef U16_t              DeviceOwnerType_t;				        /* 使用设备类型 */
...
typedef U8_t               ContainerOperatePos_t;                               /* 操作位置(1－票箱1（发售）2－票箱2（发售）3－票箱1（回收）4－票箱2（回收）5-废票箱 */

typedef U8_t               CoinBoxType_t;
typedef U8_t               NoteBoxType_t;
typedef U8_t               CashBoxType_t;
typedef U8_t               TicketBoxType_t;
