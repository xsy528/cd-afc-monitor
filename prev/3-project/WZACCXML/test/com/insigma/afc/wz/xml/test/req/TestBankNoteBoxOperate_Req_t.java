...
import com.insigma.acc.wz.xdr.message.req.BankNoteBoxOperate_Req_t;
import com.insigma.acc.wz.xdr.message.req.BankNoteBoxOperate_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.BankNoteBoxOperate_Req_t_Encoder;
 ...
 * 
 */
public class TestBankNoteBoxOperate_Req_t {
	public static void main(String[] args) {
		TestBankNoteBoxOperate_Req_t bankNoteBoxOperate = new TestBankNoteBoxOperate_Req_t();
		bankNoteBoxOperate.testModel();
	}
	...
	private void testModel() {
		...
		BankNoteBoxOperate_Req_t file = generateBean();
		...
		// decode
		BankNoteBoxOperate_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		BankNoteBoxOperate_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private BankNoteBoxOperate_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			BankNoteBoxOperate_Req_t file1 = new BankNoteBoxOperate_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, BankNoteBoxOperate_Req_t file) {
		...
		try {
			...
			new BankNoteBoxOperate_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private BankNoteBoxOperate_Req_t generateBean() {
		//test:
		BankNoteBoxOperate_Req_t dataFile_t = new BankNoteBoxOperate_Req_t();
