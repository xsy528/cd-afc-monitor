...
import com.insigma.acc.wz.xdr.message.req.CashBoxQuery_Req_t;
import com.insigma.acc.wz.xdr.message.req.CashBoxQuery_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.CashBoxQuery_Req_t_Encoder;
 ...
 * 
 */
public class TestCASHBOXQuery_Req_t {
	public static void main(String[] args) {
		TestCASHBOXQuery_Req_t cashBoxQuery = new TestCASHBOXQuery_Req_t();
		cashBoxQuery.testModel();
	}
	...
	private void testModel() {
		...
		CashBoxQuery_Req_t file = generateBean();
		...
		// decode
		CashBoxQuery_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CashBoxQuery_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CashBoxQuery_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CashBoxQuery_Req_t file1 = new CashBoxQuery_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CashBoxQuery_Req_t file) {
		...
		try {
			...
			new CashBoxQuery_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CashBoxQuery_Req_t generateBean() {
		//test:
		CashBoxQuery_Req_t dataFile_t = new CashBoxQuery_Req_t();
