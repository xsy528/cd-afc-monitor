...
import com.insigma.acc.wz.xdr.message.req.CashBoxUPLoad_Req_t;
import com.insigma.acc.wz.xdr.message.req.CashBoxUPLoad_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.CashBoxUPLoad_Req_t_Encoder;
 ...
 * 
 */
public class TestCASHBOXUPLoad_Req_t {
	public static void main(String[] args) {
		TestCASHBOXUPLoad_Req_t cashBoxUpLoad = new TestCASHBOXUPLoad_Req_t();
		cashBoxUpLoad.testModel();
	}
	...
	private void testModel() {
		...
		CashBoxUPLoad_Req_t file = generateBean();
		...
		// decode
		CashBoxUPLoad_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CashBoxUPLoad_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CashBoxUPLoad_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CashBoxUPLoad_Req_t file1 = new CashBoxUPLoad_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CashBoxUPLoad_Req_t file) {
		...
		try {
			...
			new CashBoxUPLoad_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CashBoxUPLoad_Req_t generateBean() {
		//test:
		CashBoxUPLoad_Req_t dataFile_t = new CashBoxUPLoad_Req_t();
