...
import com.insigma.acc.wz.xdr.message.req.CardAccountQuery_Req_t;
import com.insigma.acc.wz.xdr.message.req.CardAccountQuery_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.CardAccountQuery_Req_t_Encoder;
 ...
 *
 */
public class TestCardAccountQuery_Req_t {

	public static void main(String[] args) {
		TestCardAccountQuery_Req_t cardAccountQuery = new TestCardAccountQuery_Req_t();
		cardAccountQuery.testModel();
	}
	...
	private void testModel() {
		...
		CardAccountQuery_Req_t file = generateBean();
		...
		// decode
		CardAccountQuery_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CardAccountQuery_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CardAccountQuery_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CardAccountQuery_Req_t file1 = new CardAccountQuery_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CardAccountQuery_Req_t file) {
		...
		try {
			...
			new CardAccountQuery_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CardAccountQuery_Req_t generateBean() {
		//test:
		CardAccountQuery_Req_t dataFile_t = new CardAccountQuery_Req_t();
