...
import com.insigma.acc.wz.xdr.message.req.CardAccountSend_Req_t;
import com.insigma.acc.wz.xdr.message.req.CardAccountSend_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.CardAccountSend_Req_t_Encoder;
 ...
 *
 */
public class TestCardAccountSend_Req_t {

	public static void main(String[] args) {
		TestCardAccountSend_Req_t cardAccountSend = new TestCardAccountSend_Req_t();
		cardAccountSend.testModel();
	}
	...
	private void testModel() {
		...
		CardAccountSend_Req_t file = generateBean();
		...
		// decode
		CardAccountSend_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CardAccountSend_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CardAccountSend_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CardAccountSend_Req_t file1 = new CardAccountSend_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CardAccountSend_Req_t file) {
		...
		try {
			...
			new CardAccountSend_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CardAccountSend_Req_t generateBean() {
		//test:
		CardAccountSend_Req_t dataFile_t = new CardAccountSend_Req_t();
