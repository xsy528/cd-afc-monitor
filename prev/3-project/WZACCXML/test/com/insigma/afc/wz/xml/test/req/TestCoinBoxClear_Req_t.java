...
import com.insigma.acc.wz.xdr.message.req.CoinBoxClear_Req_t;
import com.insigma.acc.wz.xdr.message.req.CoinBoxClear_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.CoinBoxClear_Req_t_Encoder;
 ...
 * 
 */
public class TestCoinBoxClear_Req_t {
	public static void main(String[] args) {
		TestCoinBoxClear_Req_t coinBoxClear = new TestCoinBoxClear_Req_t();
		coinBoxClear.testModel();
	}
	...
	private void testModel() {
		...
		CoinBoxClear_Req_t file = generateBean();
		...
		// decode
		CoinBoxClear_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CoinBoxClear_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CoinBoxClear_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CoinBoxClear_Req_t file1 = new CoinBoxClear_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CoinBoxClear_Req_t file) {
		...
		try {
			...
			new CoinBoxClear_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CoinBoxClear_Req_t generateBean() {
		//test:
		CoinBoxClear_Req_t dataFile_t = new CoinBoxClear_Req_t();
