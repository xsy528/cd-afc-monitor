...
import com.insigma.acc.wz.xdr.message.req.CoinBoxOperate_Req_t;
import com.insigma.acc.wz.xdr.message.req.CoinBoxOperate_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.CoinBoxOperate_Req_t_Encoder;
 ...
 * 
 */
public class TestCoinBoxOperate_Req_t {
	public static void main(String[] args) {
		TestCoinBoxOperate_Req_t coinBoxOperate = new TestCoinBoxOperate_Req_t();
		coinBoxOperate.testModel();
	}
	...
	private void testModel() {
		...
		CoinBoxOperate_Req_t file = generateBean();
		...
		// decode
		CoinBoxOperate_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CoinBoxOperate_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CoinBoxOperate_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CoinBoxOperate_Req_t file1 = new CoinBoxOperate_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CoinBoxOperate_Req_t file) {
		...
		try {
			...
			new CoinBoxOperate_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CoinBoxOperate_Req_t generateBean() {
		//test:
		CoinBoxOperate_Req_t dataFile_t = new CoinBoxOperate_Req_t();
