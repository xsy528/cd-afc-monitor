...
import com.insigma.acc.wz.xdr.message.req.CommConnectApply_Req_t;
import com.insigma.acc.wz.xdr.message.req.CommConnectApply_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.CommConnectApply_Req_t_Encoder;
 ...
 * 
 */
public class TestCommConnectApply_Req_t {
	public static void main(String[] args) {
		TestCommConnectApply_Req_t commConnectApply = new TestCommConnectApply_Req_t();
		commConnectApply.testModel();
	}
	...
	private void testModel() {
		...
		CommConnectApply_Req_t file = generateBean();
		...
		// decode
		CommConnectApply_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CommConnectApply_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CommConnectApply_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CommConnectApply_Req_t file1 = new CommConnectApply_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CommConnectApply_Req_t file) {
		...
		try {
			...
			IXDREncoder encoder = new WZXDREncoder(xdrStream);
			new CommConnectApply_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CommConnectApply_Req_t generateBean() {
		//test:
		CommConnectApply_Req_t dataFile_t = new CommConnectApply_Req_t();
