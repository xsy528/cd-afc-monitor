...
import com.insigma.acc.wz.xdr.message.req.CurFileUploadNotify_Req_t;
import com.insigma.acc.wz.xdr.message.req.CurFileUploadNotify_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.CurFileUploadNotify_Req_t_Encoder;
 ...
 * 
 */
public class TestCurFileUploadNotify_Req_t {
	public static void main(String[] args) {
		TestCurFileUploadNotify_Req_t curFileUploadNotify = new TestCurFileUploadNotify_Req_t();
		curFileUploadNotify.testModel();
	}
	...
	private void testModel() {
		...
		CurFileUploadNotify_Req_t file = generateBean();
		...
		// decode
		CurFileUploadNotify_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CurFileUploadNotify_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CurFileUploadNotify_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CurFileUploadNotify_Req_t file1 = new CurFileUploadNotify_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CurFileUploadNotify_Req_t file) {
		...
		try {
			...
			new CurFileUploadNotify_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CurFileUploadNotify_Req_t generateBean() {
		//test:
		CurFileUploadNotify_Req_t dataFile_t = new CurFileUploadNotify_Req_t();
