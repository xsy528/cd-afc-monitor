...
import com.insigma.acc.wz.xdr.message.req.EventUpload_Req_t;
import com.insigma.acc.wz.xdr.message.req.EventUpload_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.EventUpload_Req_t_Encoder;
 ...
 * 
 */
public class TestEventUpload_Req_t {
	public static void main(String[] args) {
		TestEventUpload_Req_t sleControlCmd = new TestEventUpload_Req_t();
	...
	private void testModel() {
		...
		EventUpload_Req_t file = generateBean();
		...
		// decode
		EventUpload_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		EventUpload_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private EventUpload_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			EventUpload_Req_t file1 = new EventUpload_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, EventUpload_Req_t file) {
		...
		try {
			...
			new EventUpload_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private EventUpload_Req_t generateBean() {
		//test:
		EventUpload_Req_t dataFile_t = new EventUpload_Req_t();
