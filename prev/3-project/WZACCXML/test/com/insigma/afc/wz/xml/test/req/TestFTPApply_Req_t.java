...
import com.insigma.acc.wz.xdr.message.req.FTPApply_Req_t;
import com.insigma.acc.wz.xdr.message.req.FTPApply_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.FTPApply_Req_t_Encoder;
 ...
 * 
 */
public class TestFTPApply_Req_t {
	public static void main(String[] args) {
		TestFTPApply_Req_t ftpApply = new TestFTPApply_Req_t();
		ftpApply.testModel();
	}
	...
	private void testModel() {
		...
		FTPApply_Req_t file = generateBean();
		...
		// decode
		FTPApply_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		FTPApply_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private FTPApply_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			FTPApply_Req_t file1 = new FTPApply_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, FTPApply_Req_t file) {
		...
		try {
			...
			new FTPApply_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private FTPApply_Req_t generateBean() {
		//test:
		FTPApply_Req_t dataFile_t = new FTPApply_Req_t();
