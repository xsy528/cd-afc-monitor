...
import com.insigma.acc.wz.xdr.message.req.ModeChangeBroadcast_Req_t;
import com.insigma.acc.wz.xdr.message.req.ModeChangeBroadcast_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.ModeChangeBroadcast_Req_t_Encoder;
 ...
 * 
 */
public class TestModeChangeBroadcast_Req_t {
	public static void main(String[] args) {
		TestModeChangeBroadcast_Req_t modeChangeBroadcast = new TestModeChangeBroadcast_Req_t();
		modeChangeBroadcast.testModel();
	}
	...
	private void testModel() {
		...
		ModeChangeBroadcast_Req_t file = generateBean();
		...
		// decode
		ModeChangeBroadcast_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		ModeChangeBroadcast_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private ModeChangeBroadcast_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			ModeChangeBroadcast_Req_t file1 = new ModeChangeBroadcast_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, ModeChangeBroadcast_Req_t file) {
		...
		try {
			...
			new ModeChangeBroadcast_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private ModeChangeBroadcast_Req_t generateBean() {
		//test:
		ModeChangeBroadcast_Req_t dataFile_t = new ModeChangeBroadcast_Req_t();
