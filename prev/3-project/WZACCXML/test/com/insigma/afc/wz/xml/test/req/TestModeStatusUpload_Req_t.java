...
import com.insigma.acc.wz.xdr.message.req.ModeStatusUpload_Req_t;
import com.insigma.acc.wz.xdr.message.req.ModeStatusUpload_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.ModeStatusUpload_Req_t_Encoder;
 ...
 * 
 */
public class TestModeStatusUpload_Req_t {
	public static void main(String[] args) {
		TestModeStatusUpload_Req_t modeStatusUpload = new TestModeStatusUpload_Req_t();
		modeStatusUpload.testModel();
	}
	...
	private void testModel() {
		...
		ModeStatusUpload_Req_t file = generateBean();
		...
		// decode
		ModeStatusUpload_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		ModeStatusUpload_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private ModeStatusUpload_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			ModeStatusUpload_Req_t file1 = new ModeStatusUpload_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, ModeStatusUpload_Req_t file) {
		...
		try {
			...
			new ModeStatusUpload_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private ModeStatusUpload_Req_t generateBean() {
		//test:
		ModeStatusUpload_Req_t dataFile_t = new ModeStatusUpload_Req_t();
