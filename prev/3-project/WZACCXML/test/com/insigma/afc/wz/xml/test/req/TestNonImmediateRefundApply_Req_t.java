...
import com.insigma.acc.wz.xdr.message.req.NonImmediateRefundApply_Req_t;
import com.insigma.acc.wz.xdr.message.req.NonImmediateRefundApply_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.NonImmediateRefundApply_Req_t_Encoder;
 ...
 *
 */
public class TestNonImmediateRefundApply_Req_t {

	public static void main(String[] args) {
		TestNonImmediateRefundApply_Req_t nonImmediateRefundApply = new TestNonImmediateRefundApply_Req_t();
		nonImmediateRefundApply.testModel();
	}
	...
	private void testModel() {
		...
		NonImmediateRefundApply_Req_t file = generateBean();
		...
		// decode
		NonImmediateRefundApply_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		NonImmediateRefundApply_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private NonImmediateRefundApply_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			NonImmediateRefundApply_Req_t file1 = new NonImmediateRefundApply_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, NonImmediateRefundApply_Req_t file) {
		...
		try {
			...
			new NonImmediateRefundApply_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private NonImmediateRefundApply_Req_t generateBean() {
		//test:
		NonImmediateRefundApply_Req_t dataFile_t = new NonImmediateRefundApply_Req_t();

		dataFile_t.setDeviceID(0);
		dataFile_t.setApplyBillCode(null);
