...
import com.insigma.acc.wz.xdr.message.req.NonImmediateRefundQuery_Req_t;
import com.insigma.acc.wz.xdr.message.req.NonImmediateRefundQuery_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.NonImmediateRefundQuery_Req_t_Encoder;
 ...
 *
 */
public class TestNonImmediateRefundQuery_Req_t {

	public static void main(String[] args) {
		TestNonImmediateRefundQuery_Req_t nonImmediateRefundQuery = new TestNonImmediateRefundQuery_Req_t();
		nonImmediateRefundQuery.testModel();
	}
	...
	private void testModel() {
		...
		NonImmediateRefundQuery_Req_t file = generateBean();
		...
		// decode
		NonImmediateRefundQuery_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		NonImmediateRefundQuery_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private NonImmediateRefundQuery_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			NonImmediateRefundQuery_Req_t file1 = new NonImmediateRefundQuery_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, NonImmediateRefundQuery_Req_t file) {
		...
		try {
			...
			new NonImmediateRefundQuery_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private NonImmediateRefundQuery_Req_t generateBean() {
		//test:
		NonImmediateRefundQuery_Req_t dataFile_t = new NonImmediateRefundQuery_Req_t();
