...
import com.insigma.acc.wz.xdr.message.req.NoteBoxClear_Req_t;
import com.insigma.acc.wz.xdr.message.req.NoteBoxClear_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.NoteBoxClear_Req_t_Encoder;
 ...
 * 
 */
public class TestNoteBoxClear_Req_t {
	public static void main(String[] args) {
		TestNoteBoxClear_Req_t noteBoxClear = new TestNoteBoxClear_Req_t();
		noteBoxClear.testModel();
	}
	...
	private void testModel() {
		...
		NoteBoxClear_Req_t file = generateBean();
		...
		// decode
		NoteBoxClear_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		NoteBoxClear_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private NoteBoxClear_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			NoteBoxClear_Req_t file1 = new NoteBoxClear_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, NoteBoxClear_Req_t file) {
		...
		try {
			...
			new NoteBoxClear_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private NoteBoxClear_Req_t generateBean() {
		//test:
		NoteBoxClear_Req_t dataFile_t = new NoteBoxClear_Req_t();
