...
import com.insigma.acc.wz.xdr.message.req.OperatorWork_Req_t;
import com.insigma.acc.wz.xdr.message.req.OperatorWork_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.OperatorWork_Req_t_Encoder;
 ...
 * 
 */
public class TestOperatorWork_Req_t {
	public static void main(String[] args) {
		TestOperatorWork_Req_t operatorWork = new TestOperatorWork_Req_t();
		operatorWork.testModel();
	}
	...
	private void testModel() {
		...
		OperatorWork_Req_t file = generateBean();
		...
		// decode
		OperatorWork_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		OperatorWork_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private OperatorWork_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			OperatorWork_Req_t file1 = new OperatorWork_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, OperatorWork_Req_t file) {
		...
		try {
			...
			new OperatorWork_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private OperatorWork_Req_t generateBean() {
		//test:
		OperatorWork_Req_t dataFile_t = new OperatorWork_Req_t();
