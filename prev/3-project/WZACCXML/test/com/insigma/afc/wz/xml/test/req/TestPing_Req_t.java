...
import com.insigma.acc.wz.xdr.message.req.Ping_Req_t;
import com.insigma.acc.wz.xdr.message.req.Ping_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.Ping_Req_t_Encoder;
 ...
 * 
 */
public class TestPing_Req_t {
	public static void main(String[] args) {
		TestPing_Req_t ping = new TestPing_Req_t();
		ping.testModel();
	}
	...
	private void testModel() {
		...
		// decode
		Ping_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		Ping_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private Ping_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			Ping_Req_t file1 = new Ping_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, Ping_Req_t file) {
		...
		try {
			...
			new Ping_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private Ping_Req_t generateBean() {
		//test:
		Ping_Req_t dataFile_t = new Ping_Req_t();
