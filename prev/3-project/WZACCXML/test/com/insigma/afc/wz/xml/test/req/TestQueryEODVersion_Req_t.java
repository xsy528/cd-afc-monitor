...
import com.insigma.acc.wz.xdr.message.req.QueryEODVersion_Req_t;
import com.insigma.acc.wz.xdr.message.req.QueryEODVersion_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.QueryEODVersion_Req_t_Encoder;
 ...
 * 
 */
public class TestQueryEODVersion_Req_t {
	public static void main(String[] args) {
		TestQueryEODVersion_Req_t queryEODVersion = new TestQueryEODVersion_Req_t();
		queryEODVersion.testModel();
	}
	...
	private void testModel() {
		...
		QueryEODVersion_Req_t file = generateBean();
		...
		// decode
		QueryEODVersion_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		QueryEODVersion_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private QueryEODVersion_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			QueryEODVersion_Req_t file1 = new QueryEODVersion_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, QueryEODVersion_Req_t file) {
		...
		try {
			...
			new QueryEODVersion_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private QueryEODVersion_Req_t generateBean() {
		//test:
		QueryEODVersion_Req_t dataFile_t = new QueryEODVersion_Req_t();
