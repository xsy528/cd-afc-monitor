...
import com.insigma.acc.wz.xdr.message.req.QueryFileVersion_Req_t;
import com.insigma.acc.wz.xdr.message.req.QueryFileVersion_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.QueryFileVersion_Req_t_Encoder;
 ...
 * 
 */
public class TestQueryFileVersion_Req_t {
	public static void main(String[] args) {
		TestQueryFileVersion_Req_t queryFileVersion = new TestQueryFileVersion_Req_t();
		queryFileVersion.testModel();
	}
	...
	private void testModel() {
		...
		QueryFileVersion_Req_t file = generateBean();
		...
		// decode
		QueryFileVersion_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		QueryFileVersion_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private QueryFileVersion_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			QueryFileVersion_Req_t file1 = new QueryFileVersion_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, QueryFileVersion_Req_t file) {
		...
		try {
			...
			new QueryFileVersion_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private QueryFileVersion_Req_t generateBean() {
		//test:
		QueryFileVersion_Req_t dataFile_t = new QueryFileVersion_Req_t();
