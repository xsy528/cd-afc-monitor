...
import com.insigma.acc.wz.xdr.message.req.QueryMode_Req_t;
import com.insigma.acc.wz.xdr.message.req.QueryMode_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.QueryMode_Req_t_Encoder;
 ...
 * 
 */
public class TestQueryMode_Req_t {
	public static void main(String[] args) {
		TestQueryMode_Req_t queryMode = new TestQueryMode_Req_t();
		queryMode.testModel();
	}
	...
	private void testModel() {
		...
		QueryMode_Req_t file = generateBean();
		...
		// decode
		QueryMode_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		QueryMode_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private QueryMode_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			QueryMode_Req_t file1 = new QueryMode_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, QueryMode_Req_t file) {
		...
		try {
			...
			new QueryMode_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private QueryMode_Req_t generateBean() {
		//test:
		QueryMode_Req_t dataFile_t = new QueryMode_Req_t();
