...
import com.insigma.acc.wz.xdr.message.req.QuerySN_Req_t;
import com.insigma.acc.wz.xdr.message.req.QuerySN_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.QuerySN_Req_t_Encoder;
 ...
 * 
 */
public class TestQuerySN_Req_t {
	public static void main(String[] args) {
		TestQuerySN_Req_t querySN = new TestQuerySN_Req_t();
		querySN.testModel();
	}
	...
	private void testModel() {
		...
		QuerySN_Req_t file = generateBean();
		...
		// decode
		QuerySN_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		QuerySN_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private QuerySN_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			QuerySN_Req_t file1 = new QuerySN_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, QuerySN_Req_t file) {
		...
		try {
			...
			new QuerySN_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private QuerySN_Req_t generateBean() {
		//test:
		QuerySN_Req_t dataFile_t = new QuerySN_Req_t();
