...
import com.insigma.acc.wz.xdr.message.req.SendEmergencyMode_Req_t;
import com.insigma.acc.wz.xdr.message.req.SendEmergencyMode_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.SendEmergencyMode_Req_t_Encoder;
 ...
 * 
 */
public class TestSendEmergencyMode_Req_t {
	public static void main(String[] args) {
		TestSendEmergencyMode_Req_t sendEmergencyMode = new TestSendEmergencyMode_Req_t();
		sendEmergencyMode.testModel();
	}
	...
	private void testModel() {
		...
		SendEmergencyMode_Req_t file = generateBean();
		...
		// decode
		SendEmergencyMode_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		SendEmergencyMode_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private SendEmergencyMode_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			SendEmergencyMode_Req_t file1 = new SendEmergencyMode_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, SendEmergencyMode_Req_t file) {
		...
		try {
			...
			new SendEmergencyMode_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private SendEmergencyMode_Req_t generateBean() {
		//test:
		SendEmergencyMode_Req_t dataFile_t = new SendEmergencyMode_Req_t();
