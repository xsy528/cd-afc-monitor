...
import com.insigma.acc.wz.xdr.message.req.SendOtherMode_Req_t;
import com.insigma.acc.wz.xdr.message.req.SendOtherMode_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.SendOtherMode_Req_t_Encoder;
 ...
 * 
 */
public class TestSendOtherMode_Req_t {
	public static void main(String[] args) {
		TestSendOtherMode_Req_t sendOtherMode = new TestSendOtherMode_Req_t();
		sendOtherMode.testModel();
	}
	...
	private void testModel() {
		...
		SendOtherMode_Req_t file = generateBean();
		...
		// decode
		SendOtherMode_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		SendOtherMode_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private SendOtherMode_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			SendOtherMode_Req_t file1 = new SendOtherMode_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, SendOtherMode_Req_t file) {
		...
		try {
			...
			new SendOtherMode_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private SendOtherMode_Req_t generateBean() {
		//test:
		SendOtherMode_Req_t dataFile_t = new SendOtherMode_Req_t();
