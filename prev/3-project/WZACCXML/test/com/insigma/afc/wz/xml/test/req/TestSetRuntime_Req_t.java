...
import com.insigma.acc.wz.xdr.message.req.SetRuntime_Req_t;
import com.insigma.acc.wz.xdr.message.req.SetRuntime_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.SetRuntime_Req_t_Encoder;
 ...
 * 
 */
public class TestSetRuntime_Req_t {
	public static void main(String[] args) {
		TestSetRuntime_Req_t setRuntime = new TestSetRuntime_Req_t();
		setRuntime.testModel();
	}
	...
	private void testModel() {
		...
		SetRuntime_Req_t file = generateBean();
		...
		// decode
		SetRuntime_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		SetRuntime_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private SetRuntime_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			SetRuntime_Req_t file1 = new SetRuntime_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, SetRuntime_Req_t file) {
		...
		try {
			...
			new SetRuntime_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private SetRuntime_Req_t generateBean() {
		//test:
		SetRuntime_Req_t dataFile_t = new SetRuntime_Req_t();
