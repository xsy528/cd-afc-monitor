...
import com.insigma.acc.wz.xdr.message.req.SleControlCmd_Req_t;
import com.insigma.acc.wz.xdr.message.req.SleControlCmd_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.SleControlCmd_Req_t_Encoder;
 ...
 * 
 */
public class TestSleControlCmd_Req_t {
	public static void main(String[] args) {
		TestSleControlCmd_Req_t sleControlCmd = new TestSleControlCmd_Req_t();
		sleControlCmd.testModel();
	}
	...
	private void testModel() {
		...
		SleControlCmd_Req_t file = generateBean();
		...
		// decode
		SleControlCmd_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		SleControlCmd_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private SleControlCmd_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			SleControlCmd_Req_t file1 = new SleControlCmd_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, SleControlCmd_Req_t file) {
		...
		try {
			...
			new SleControlCmd_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private SleControlCmd_Req_t generateBean() {
		//test:
		SleControlCmd_Req_t dataFile_t = new SleControlCmd_Req_t();
