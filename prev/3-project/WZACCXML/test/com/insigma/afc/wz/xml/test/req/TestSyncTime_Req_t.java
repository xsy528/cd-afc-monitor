...
import com.insigma.acc.wz.xdr.message.req.SyncTime_Req_t;
import com.insigma.acc.wz.xdr.message.req.SyncTime_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.SyncTime_Req_t_Encoder;
 ...
 * 
 */
public class TestSyncTime_Req_t {
	public static void main(String[] args) {
		TestSyncTime_Req_t syncTime = new TestSyncTime_Req_t();
		syncTime.testModel();
	}
	...
	private void testModel() {
		...
		SyncTime_Req_t file = generateBean();
		...
		// decode
		SyncTime_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		SyncTime_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private SyncTime_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			SyncTime_Req_t file1 = new SyncTime_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, SyncTime_Req_t file) {
		...
		try {
			...
			new SyncTime_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private SyncTime_Req_t generateBean() {
		//test:
		SyncTime_Req_t dataFile_t = new SyncTime_Req_t();
