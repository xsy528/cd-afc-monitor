...
import com.insigma.acc.wz.xdr.message.req.Text_Req_t;
import com.insigma.acc.wz.xdr.message.req.Text_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.Text_Req_t_Encoder;
 ...
 * 
 */
public class TestText_Req_t {
	public static void main(String[] args) {
		TestText_Req_t text = new TestText_Req_t();
		text.testModel();
	}
	...
	private void testModel() {
		...
		// decode
		Text_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		Text_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private Text_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			Text_Req_t file1 = new Text_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, Text_Req_t file) {
		...
		try {
			...
			new Text_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private Text_Req_t generateBean() {
		//test:
		Text_Req_t dataFile_t = new Text_Req_t();
