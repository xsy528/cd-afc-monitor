...
import com.insigma.acc.wz.xdr.message.req.TicketAllocApply_Req_t;
import com.insigma.acc.wz.xdr.message.req.TicketAllocApply_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.TicketAllocApply_Req_t_Encoder;
 ...
 * 
 */
public class TestTicketAllocApply_Req_t {
	public static void main(String[] args) {
		TestTicketAllocApply_Req_t ticketAllocApply = new TestTicketAllocApply_Req_t();
		ticketAllocApply.testModel();
	}
	...
	private void testModel() {
		...
		TicketAllocApply_Req_t file = generateBean();
		...
		// decode
		TicketAllocApply_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		TicketAllocApply_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private TicketAllocApply_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			TicketAllocApply_Req_t file1 = new TicketAllocApply_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, TicketAllocApply_Req_t file) {
		...
		try {
			...
			new TicketAllocApply_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private TicketAllocApply_Req_t generateBean() {
		//test:
		TicketAllocApply_Req_t dataFile_t = new TicketAllocApply_Req_t();
