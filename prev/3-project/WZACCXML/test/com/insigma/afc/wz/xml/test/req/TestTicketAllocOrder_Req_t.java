...
import com.insigma.acc.wz.xdr.message.req.TicketAllocOrder_Req_t;
import com.insigma.acc.wz.xdr.message.req.TicketAllocOrder_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.TicketAllocOrder_Req_t_Encoder;
 ...
 * 
 */
public class TestTicketAllocOrder_Req_t {
	public static void main(String[] args) {
		TestTicketAllocOrder_Req_t ticketAllocOrder = new TestTicketAllocOrder_Req_t();
		ticketAllocOrder.testModel();
	}
	...
	private void testModel() {
		...
		TicketAllocOrder_Req_t file = generateBean();
		...
		// decode
		TicketAllocOrder_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		TicketAllocOrder_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private TicketAllocOrder_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			TicketAllocOrder_Req_t file1 = new TicketAllocOrder_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, TicketAllocOrder_Req_t file) {
		...
		try {
			...
			new TicketAllocOrder_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private TicketAllocOrder_Req_t generateBean() {
		//test:
		TicketAllocOrder_Req_t dataFile_t = new TicketAllocOrder_Req_t();
