...
import com.insigma.acc.wz.xdr.message.req.TicketBoxOperate_Req_t;
import com.insigma.acc.wz.xdr.message.req.TicketBoxOperate_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.TicketBoxOperate_Req_t_Encoder;
 ...
 * 
 */
public class TestTicketBoxOperate_Req_t {
	public static void main(String[] args) {
		TestTicketBoxOperate_Req_t ticketBoxOperate = new TestTicketBoxOperate_Req_t();
		ticketBoxOperate.testModel();
	}
	...
	private void testModel() {
		...
		TicketBoxOperate_Req_t file = generateBean();
		...
		// decode
		TicketBoxOperate_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		TicketBoxOperate_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private TicketBoxOperate_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			TicketBoxOperate_Req_t file1 = new TicketBoxOperate_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, TicketBoxOperate_Req_t file) {
		...
		try {
			...
			new TicketBoxOperate_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private TicketBoxOperate_Req_t generateBean() {
		//test:
		TicketBoxOperate_Req_t dataFile_t = new TicketBoxOperate_Req_t();
