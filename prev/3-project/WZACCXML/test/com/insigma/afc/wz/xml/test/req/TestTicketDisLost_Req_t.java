...
import com.insigma.acc.wz.xdr.message.req.TicketDisLost_Req_t;
import com.insigma.acc.wz.xdr.message.req.TicketDisLost_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.TicketDisLost_Req_t_Encoder;
 ...
 *
 */
public class TestTicketDisLost_Req_t {

	public static void main(String[] args) {
		TestTicketDisLost_Req_t ticketDisLost = new TestTicketDisLost_Req_t();
		ticketDisLost.testModel();
	}
	...
	private void testModel() {
		...
		TicketDisLost_Req_t file = generateBean();
		...
		// decode
		TicketDisLost_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		TicketDisLost_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private TicketDisLost_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			TicketDisLost_Req_t file1 = new TicketDisLost_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, TicketDisLost_Req_t file) {
		...
		try {
			...
			new TicketDisLost_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private TicketDisLost_Req_t generateBean() {
		//test:
		TicketDisLost_Req_t dataFile_t = new TicketDisLost_Req_t();
