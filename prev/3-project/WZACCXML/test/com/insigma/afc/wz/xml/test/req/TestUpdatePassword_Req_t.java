...
import com.insigma.acc.wz.xdr.message.req.UpdatePassword_Req_t;
import com.insigma.acc.wz.xdr.message.req.UpdatePassword_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.UpdatePassword_Req_t_Encoder;
 ...
 *
 */
public class TestUpdatePassword_Req_t {

	public static void main(String[] args) {
		TestUpdatePassword_Req_t updatePassword = new TestUpdatePassword_Req_t();
	...
	private void testModel() {
		...
		UpdatePassword_Req_t file = generateBean();
		...
		// decode
		UpdatePassword_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		UpdatePassword_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private UpdatePassword_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			UpdatePassword_Req_t file1 = new UpdatePassword_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, UpdatePassword_Req_t file) {
		...
		try {
			...
			new UpdatePassword_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private UpdatePassword_Req_t generateBean() {
		//test:
		UpdatePassword_Req_t dataFile_t = new UpdatePassword_Req_t();
