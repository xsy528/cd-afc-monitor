...
import com.insigma.acc.wz.xdr.message.req.UploadEODVersion_Req_t;
import com.insigma.acc.wz.xdr.message.req.UploadEODVersion_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.UploadEODVersion_Req_t_Encoder;
 ...
 * 
 */
public class TestUploadEODVersion_Req_t {
	public static void main(String[] args) {
		TestUploadEODVersion_Req_t uploadEODVersion = new TestUploadEODVersion_Req_t();
		uploadEODVersion.testModel();
	}
	...
	private void testModel() {
		...
		UploadEODVersion_Req_t file = generateBean();
		...
		// decode
		UploadEODVersion_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		UploadEODVersion_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private UploadEODVersion_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			UploadEODVersion_Req_t file1 = new UploadEODVersion_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, UploadEODVersion_Req_t file) {
		...
		try {
			...
			new UploadEODVersion_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private UploadEODVersion_Req_t generateBean() {
		//test:
		UploadEODVersion_Req_t dataFile_t = new UploadEODVersion_Req_t();
