...
import com.insigma.acc.wz.xdr.message.req.UploadParameterVersion_Req_t;
import com.insigma.acc.wz.xdr.message.req.UploadParameterVersion_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.UploadParameterVersion_Req_t_Encoder;
 ...
 * 
 */
public class TestUploadParameterVersion_Req_t {
	public static void main(String[] args) {
		TestUploadParameterVersion_Req_t uploadParameterVersion = new TestUploadParameterVersion_Req_t();
		uploadParameterVersion.testModel();
	}
	...
	private void testModel() {
		...
		UploadParameterVersion_Req_t file = generateBean();
		...
		// decode
		UploadParameterVersion_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		UploadParameterVersion_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private UploadParameterVersion_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			UploadParameterVersion_Req_t file1 = new UploadParameterVersion_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, UploadParameterVersion_Req_t file) {
		...
		try {
			...
			new UploadParameterVersion_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private UploadParameterVersion_Req_t generateBean() {
		//test:
		UploadParameterVersion_Req_t dataFile_t = new UploadParameterVersion_Req_t();
