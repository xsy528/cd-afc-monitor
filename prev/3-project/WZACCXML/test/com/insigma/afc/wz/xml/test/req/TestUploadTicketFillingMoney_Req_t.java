...
import com.insigma.acc.wz.xdr.message.req.UploadTicketFillingMoney_Req_t;
import com.insigma.acc.wz.xdr.message.req.UploadTicketFillingMoney_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.UploadTicketFillingMoney_Req_t_Encoder;
 ...
 *
 */
public class TestUploadTicketFillingMoney_Req_t {

	public static void main(String[] args) {
		TestUploadTicketFillingMoney_Req_t uploadTicketFillingMoney = new TestUploadTicketFillingMoney_Req_t();
		uploadTicketFillingMoney.testModel();
	}
	...
	private void testModel() {
		...
		UploadTicketFillingMoney_Req_t file = generateBean();
		...
		// decode
		UploadTicketFillingMoney_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		UploadTicketFillingMoney_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private UploadTicketFillingMoney_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			UploadTicketFillingMoney_Req_t file1 = new UploadTicketFillingMoney_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, UploadTicketFillingMoney_Req_t file) {
		...
		try {
			...
			new UploadTicketFillingMoney_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private UploadTicketFillingMoney_Req_t generateBean() {
		//test:
		UploadTicketFillingMoney_Req_t dataFile_t = new UploadTicketFillingMoney_Req_t();
