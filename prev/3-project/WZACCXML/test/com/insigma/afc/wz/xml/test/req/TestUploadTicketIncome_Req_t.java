...
import com.insigma.acc.wz.xdr.message.req.UploadTicketIncome_Req_t;
import com.insigma.acc.wz.xdr.message.req.UploadTicketIncome_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.UploadTicketIncome_Req_t_Encoder;
 ...
 *
 */
public class TestUploadTicketIncome_Req_t {
	public static void main(String[] args) {
		TestUploadTicketIncome_Req_t uploadTicketIncome = new TestUploadTicketIncome_Req_t();
		uploadTicketIncome.testModel();
	}
	...
	private void testModel() {
		...
		UploadTicketIncome_Req_t file = generateBean();
		...
		// decode
		UploadTicketIncome_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		UploadTicketIncome_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private UploadTicketIncome_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			UploadTicketIncome_Req_t file1 = new UploadTicketIncome_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, UploadTicketIncome_Req_t file) {
		...
		try {
			...
			new UploadTicketIncome_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private UploadTicketIncome_Req_t generateBean() {
		//test:
		UploadTicketIncome_Req_t dataFile_t = new UploadTicketIncome_Req_t();
