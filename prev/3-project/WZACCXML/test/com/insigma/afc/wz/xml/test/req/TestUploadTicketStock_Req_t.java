...
import com.insigma.acc.wz.xdr.message.req.UploadTicketStock_Req_t;
import com.insigma.acc.wz.xdr.message.req.UploadTicketStock_Req_t_Decoder;
import com.insigma.acc.wz.xdr.message.req.UploadTicketStock_Req_t_Encoder;
 ...
 * 
 */
public class TestUploadTicketStock_Req_t {
	public static void main(String[] args) {
		TestUploadTicketStock_Req_t uploadTicketStock = new TestUploadTicketStock_Req_t();
		uploadTicketStock.testModel();
	}
	...
	private void testModel() {
		...
		UploadTicketStock_Req_t file = generateBean();
		...
		// decode
		UploadTicketStock_Req_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		UploadTicketStock_Req_t file_t2 = fileDecode(filePath, fileName2);
	...
	private UploadTicketStock_Req_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			UploadTicketStock_Req_t file1 = new UploadTicketStock_Req_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, UploadTicketStock_Req_t file) {
		...
		try {
			...
			new UploadTicketStock_Req_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private UploadTicketStock_Req_t generateBean() {
		//test:
		UploadTicketStock_Req_t dataFile_t = new UploadTicketStock_Req_t();
