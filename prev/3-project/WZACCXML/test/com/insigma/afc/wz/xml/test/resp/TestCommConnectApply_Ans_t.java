...
import com.insigma.acc.wz.xdr.message.resp.CommConnectApply_Ans_t;
import com.insigma.acc.wz.xdr.message.resp.CommConnectApply_Ans_t_Decoder;
import com.insigma.acc.wz.xdr.message.resp.CommConnectApply_Ans_t_Encoder;
 ...
 *
 */
public class TestCommConnectApply_Ans_t {

	public static void main(String[] args) {
		TestCommConnectApply_Ans_t commConnectApply = new TestCommConnectApply_Ans_t();
	...
	private void testModel() {
		...
		CommConnectApply_Ans_t file = generateBean();
		...
		// decode
		CommConnectApply_Ans_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		CommConnectApply_Ans_t file_t2 = fileDecode(filePath, fileName2);
	...
	private CommConnectApply_Ans_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			CommConnectApply_Ans_t file1 = new CommConnectApply_Ans_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, CommConnectApply_Ans_t file) {
		...
		try {
			...
			new CommConnectApply_Ans_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private CommConnectApply_Ans_t generateBean() {
		//test:
		CommConnectApply_Ans_t dataFile_t = new CommConnectApply_Ans_t();
