...
import com.insigma.acc.wz.xdr.message.resp.FTPApply_Ans_t;
import com.insigma.acc.wz.xdr.message.resp.FTPApply_Ans_t_Decoder;
import com.insigma.acc.wz.xdr.message.resp.FTPApply_Ans_t_Encoder;
 ...
 *
 */
public class TestFTPApply_Ans_t {

	public static void main(String[] args) {
		TestFTPApply_Ans_t ftpApply = new TestFTPApply_Ans_t();
		ftpApply.testModel();
	}
	...
	private void testModel() {
		...
		FTPApply_Ans_t file = generateBean();
		...
		// decode
		FTPApply_Ans_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		FTPApply_Ans_t file_t2 = fileDecode(filePath, fileName2);
	...
	private FTPApply_Ans_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			FTPApply_Ans_t file1 = new FTPApply_Ans_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, FTPApply_Ans_t file) {
		...
		try {
			...
			new FTPApply_Ans_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private FTPApply_Ans_t generateBean() {
		//test:
		FTPApply_Ans_t dataFile_t = new FTPApply_Ans_t();
