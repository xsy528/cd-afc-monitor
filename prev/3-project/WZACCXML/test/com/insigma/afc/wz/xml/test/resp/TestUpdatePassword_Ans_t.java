...
import com.insigma.acc.wz.xdr.message.resp.UpdatePassword_Ans_t;
import com.insigma.acc.wz.xdr.message.resp.UpdatePassword_Ans_t_Decoder;
import com.insigma.acc.wz.xdr.message.resp.UpdatePassword_Ans_t_Encoder;
 ...
 *
 */
public class TestUpdatePassword_Ans_t {

	public static void main(String[] args) {
		TestUpdatePassword_Ans_t updatePassword = new TestUpdatePassword_Ans_t();
		updatePassword.testModel();
	}
	...
	private void testModel() {
		...
		UpdatePassword_Ans_t file = generateBean();
		...
		// decode
		UpdatePassword_Ans_t file_t = fileDecode(filePath, fileName);

		// 反向encode
		fileEncoder(filePath, fileName2, file_t);

		// 反向decode
		UpdatePassword_Ans_t file_t2 = fileDecode(filePath, fileName2);
	...
	private UpdatePassword_Ans_t fileDecode(String filePath, String fileName) {
		...
		try {
			...
			UpdatePassword_Ans_t file1 = new UpdatePassword_Ans_t_Decoder().decode(decoder);
			return file1;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	private void fileEncoder(String filePath, String fileName, UpdatePassword_Ans_t file) {
		...
		try {
			...
			new UpdatePassword_Ans_t_Encoder().encode(encoder, file);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (out != null) {
	...
	private UpdatePassword_Ans_t generateBean() {
		//test:
		UpdatePassword_Ans_t dataFile_t = new UpdatePassword_Ans_t();
